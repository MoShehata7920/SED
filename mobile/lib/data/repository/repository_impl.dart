...
class RepositoryImpl implements Repository {
  ...
  @override
  Future<Either<Failure, VerifyEMail>> resetPasswordOTP(int code) async {
    if (await _networkInfo.isConnected) {
      //device is connected to the internet, call api
      try {
        final response = await _remoteDataSource.resetPasswordOTP(code);
  ...
  Future<Either<Failure, NewConversation>> newConversation(
    ...
    if (await _networkInfo.isConnected) {
      ...
      try {
        final response =
            await _remoteDataSource.newConversation(newConversationRequest);
